...
describe('TrafficConesComponent', () => {
  ...
  beforeEach(async () => {
    mockConeStateService = jasmine.createSpyObj('ConeStateService', [
      'someMethod',
    ]);
    mockStopLineService = jasmine.createSpyObj('StopLineService', [
      'callCreateStopLine',
    ]);
  ...
  describe('getConeByPosition()', () => {
    it('should return cone by position when close enough', () => {
      ...
      const result = component.getConeByPosition(
        new THREE.Vector3(1.05, 0, 1.02),
      );
  ...
  describe('loadConeModel()', () => {
    ...
    it('should reject loading if car is not defined', async () => {
      ...
      await expectAsync(component.loadConeModel(1, 5, 5)).toBeRejectedWithError(
        'Car is not defined',
      );
    });
  });
